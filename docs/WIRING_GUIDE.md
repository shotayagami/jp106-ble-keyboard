...
Row3 GP3─[LSft]─[Z]────[X]────[C]────[V]────[B]────[N]────[M]────[,]────[.]────[/]────[＼]───[↑]───[RSft]─
          │      │      │      │      │      │      │      │      │      │      │      │      │      │
Row4 GP4─[LCtl]─[Win]──[LAlt]─[無変]─[Fn]───[Space]─[Fn]──[変換]─[かな]─[RAlt]─[RCtl]─[←]───[↓]───[→]───
          │      │      │      │      │      │      │      │      │      │      │      │      │      │
...
※ Row5-7の Col13 = 空きポジション (112中3箇所が未使用)
※ Row4/Col4, Col6 = Fn キー ×2 (ファームウェア専用、HIDには送信されない)
```
...
| 無変換 | Row4, Col3 | `0x8B` | JIS固有: Fn(L)の左 |
| 変換 | Row4, Col7 | `0x8A` | JIS固有: Fn(R)の右 |
| カタカナ/ひらがな | Row4, Col8 | `0x88` | JIS固有: 変換キー右 |
...
マトリクス Row4/Col4 (Spaceの左隣) と Row4/Col6 (Spaceの右隣) に Fn キーを2個配置。
どちらのFnキーも同じ機能を持つ (matrix_fn_is_pressed() がマトリクス全体をスキャンするため)。
